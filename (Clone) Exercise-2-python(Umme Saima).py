...
aggregated_data_frame = weatherDataFrameWithYear.groupBy("year") \
    .agg(F.min("temperaturemin").alias("min_temperature"), F.max("temperaturemax").alias("max_temperature"))

sortedAggregated_data_frame = aggregated_data_frame.orderBy(F.desc("year"))


sortedAggregated_data_frame.show()
...
min2012: float = task.filter(task["year"] == 2012).select("min_temperature").first()[0]
max2016: float = task.filter(task["year"] == 2016).select("max_temperature").first()[0]
entries2018: int = task.filter(task["year"] == 2018).select("number_of_entries").first()[0]
wind2008: float = task.filter(task["year"] == 2008).select("average_wind_speed").first()[0]
...
filtered_data_frame = weatherDataFrameWithYear.filter((F.col("fog") == "Yes"))

fogCount_data_frame = filtered_data_frame.groupBy("year").agg(F.count("date").alias("fog_days_count"))

yearWithMostDaysWithFog = fogCount_data_frame.orderBy(F.desc("fog_days_count")).select("year").first()[0]
